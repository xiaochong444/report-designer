import { band, template, text } from './common';
...
export const longTextPaginationTemplate = template('long-text-pagination', 'Long Text Pagination', [
  band('lt-title', 'reportTitle', 12, [
    text('lt-title-text', 'Long Text Pagination', 0, 1, 190, 8, { font: { size: 15, bold: true }, textAlign: 'center' }),
  ]),
  band('lt-header', 'header', 8, [
    text('lt-header-text', 'Narrative rows', 0, 1, 100, 5, { font: { bold: true } }),
  ]),
  band('lt-data', 'data', 14, [
    text('lt-name', '{employees.name}', 0, 1, 36, 5, { font: { bold: true } }),
    text('lt-paragraph', paragraph, 40, 1, 136, 8, { canGrow: true }),
