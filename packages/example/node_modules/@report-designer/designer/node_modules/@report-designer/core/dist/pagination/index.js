 ...
 */
export function paginate(template, data) {
    ...
    for (const renderedPage of renderTree.pages) {
        ...
        const contentBands = renderedPage.bands.filter(b => b.type !== 'pageHeader' && b.type !== 'pageFooter');
...
function splitBandsAcrossPages(bands, maxPageHeight) {
    const groups = [];
    let currentGroup = [];
    let currentHeight = 0;
...
function toPaginatedComponent(bandY, marginLeft) {
    return (comp) => ({
        id: comp.id,
        type: comp.type,
...
export function paginatedPageHeight(page) {
