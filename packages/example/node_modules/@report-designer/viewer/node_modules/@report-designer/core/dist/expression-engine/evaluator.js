...
function evalArg(arg, ctx) {
    return evaluate(arg, ctx);
}
function evalNumericArg(arg, ctx) {
...
reg('IIF', (args, ctx) => {
    if (args.length !== 3)
        throw new Error('IIF requires exactly 3 arguments');
    return args[0] ? args[1] : args[2];
});
reg('ISNULL', (args, ctx) => {
    if (args.length < 1)
        throw new Error('ISNULL requires at least 1 argument');
...
export function evalExpression(expression, resolveField, rowIndex, variables) {
